...
function tarjetaProducto(producto) {
  const imagen = producto.imagen || 'https://via.placeholder.com/400x180?text=Sin+imagen';
...
async function cargarProductos() {
  try {
    ...
    if (!productos || productos.length === 0) {
      mostrarEstado('No hay productos cargados todavía. Agrégalos desde /admin/.', 'warning');
      return;
    }
    ...
    mostrarEstado(
      `No se pudo conectar con el backend (${API_BASE_URL}). Detalle: ${error.message}`,
      'danger',
    );
  }
}
